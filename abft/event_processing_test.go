...
import (
	...
	"github.com/greenchainearth/seed-base/inter/dag/tdag"
...
func TestSeedRandom_1(t *testing.T) {
	testSeedRandom(t, []pos.Weight{1}, 0)
}

func TestSeedRandom_big1(t *testing.T) {
	testSeedRandom(t, []pos.Weight{math.MaxUint32 / 2}, 0)
}

func TestSeedRandom_big2(t *testing.T) {
	testSeedRandom(t, []pos.Weight{math.MaxUint32 / 4, math.MaxUint32 / 4}, 0)
}

func TestSeedRandom_big3(t *testing.T) {
	testSeedRandom(t, []pos.Weight{math.MaxUint32 / 8, math.MaxUint32 / 8, math.MaxUint32 / 4}, 0)
}

func TestSeedRandom_4(t *testing.T) {
	testSeedRandom(t, []pos.Weight{1, 2, 3, 4}, 0)
}

func TestSeedRandom_3_1(t *testing.T) {
	testSeedRandom(t, []pos.Weight{1, 1, 1, 1}, 1)
}

func TestSeedRandom_67_33(t *testing.T) {
	testSeedRandom(t, []pos.Weight{33, 67}, 1)
}

func TestSeedRandom_67_33_4(t *testing.T) {
	testSeedRandom(t, []pos.Weight{11, 11, 11, 67}, 3)
}

func TestSeedRandom_67_33_5(t *testing.T) {
	testSeedRandom(t, []pos.Weight{11, 11, 11, 33, 34}, 3)
}

func TestSeedRandom_2_8_10(t *testing.T) {
	testSeedRandom(t, []pos.Weight{1, 2, 1, 2, 1, 2, 1, 2, 1, 2}, 3)
}

// TestSeed 's possibility to get consensus in general on any event order.
func testSeedRandom(t *testing.T, weights []pos.Weight, cheatersCount int) {
	assertar := assert.New(t)

	const lchCount = 3
	nodes := tdag.GenNodes(len(weights))

	lchs := make([]*TestSeed, 0, lchCount)
	inputs := make([]*EventStore, 0, lchCount)
	for i := 0; i < lchCount; i++ {
		lch, _, input := FakeSeed(nodes, weights)
...
func compareResults(t *testing.T, lchs []*TestSeed) {
	assertar := assert.New(t)
